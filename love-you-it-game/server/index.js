...
const PROPS = [
  ...
  { id:"bottles", level:"DN", x:8,  y:34, w:3.1, h:3.3, solid:1 },  // Dave's office
  ...
  { id:"binbox",  level:"UP", x:49, y:15, w:3.1, h:3.0, solid:1 },  // outside Bluebeam
  { id:"filter",  level:"UP", x:51, y:6,  w:2.6, h:3.6, solid:1 },  // supply / stairs
  ...
  { id:"truck",   level:"EXT", x:6,  y:11, w:6.0, h:3.3, cookout:1 },
...
/* Prop collision lives in its own layer rather than being written into
   the map grid. The grid is what gets sent to clients for drawing, so
   carving props into it made them render as wall-coloured blocks behind
   the artwork. Keeping the two separate means an object blocks movement
   while the floor underneath still looks like floor.                   */
for (const lv of Object.keys(LEVELS)) {
  LEVELS[lv].blocked = Array.from({ length: GH }, () => new Array(GW).fill(0));
}
for (const pr of PROPS) {
  if (!pr.solid || pr.cookout) continue;
  const b = LEVELS[pr.level].blocked;
  // footprint is the base of the object, not its full drawn height --
  // tall things like the shelf shouldn't block the floor above them
  const x0 = Math.round(pr.x - pr.w / 2 + 0.5), x1 = Math.round(pr.x + pr.w / 2 - 0.5);
  const y0 = Math.round(pr.y - Math.min(pr.h, 2) / 2 + 0.5), y1 = Math.round(pr.y + 1);
  for (let y = y0; y < y1; y++)
    for (let x = x0; x < x1; x++)
      if (b[y] && b[y][x] !== undefined) b[y][x] = 1;
}
...
function solid(level, tx, ty) {
  ...
  if (L.grid[ty][tx] === 1) return true;
  return !!(L.blocked && L.blocked[ty][tx]);   // furniture, invisible to the renderer
}
