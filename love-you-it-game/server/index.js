...
const RENOS=[
  ...
  {id:'roof',    n:'ROOF',    level:'EXT',x:9, y:24},
  {id:'siding',  n:'SIDING',  level:'EXT',x:26,y:40},
  {id:'windows', n:'WINDOWS', level:'UP', x:8, y:21},
  {id:'doors',   n:'DOORS',   level:'UP', x:52,y:39},
];
const STATIONS=[
  {id:'cyber',    n:'Cyber Attack',            level:'DN', x:40,y:8,  need:2, brk:7.6},
  {id:'printjam', n:'Print Jam',               level:'DN', x:10,y:29, need:2, brk:7.2},
  {id:'heater',   n:'Water Heater Leak',       level:'DN', x:30,y:21, need:2, brk:8.0},
  {id:'truck',    n:'Truck Service Overdue',   level:'DN', x:52,y:20, need:2, brk:8.8},
  {id:'cams',     n:'Cameras Offline',         level:'DN', x:52,y:28, need:2, brk:8.4, noPing:1},
  {id:'outlook',  n:'Outlook Crash',           level:'UP', x:31,y:8,  need:2, brk:7.2},
  {id:'toner',    n:'Toner Out',               level:'UP', x:56,y:8,  need:2, brk:7.2},
  {id:'ac',       n:'AC Not Working',          level:'UP', x:21,y:24, need:2, brk:8.8},
  {id:'bluebeam', n:'Bluebeam License Expired',level:'UP', x:54,y:23, need:2, brk:7.6},
  {id:'teams',    n:'Teams Meeting Issues',    level:'UP', x:30,y:39, need:2, brk:7.6},
];
const JAIL={level:'UP', x:11, y:39};

/* dwell tuning, carried over from the single-player build */
const ACT_R      = 1.5;   // how close you must be to work a pad
const RENO_SOLO  = 1.0;   // progress per second, one IT
const RENO_PAIR  = 3.0;   // two IT together
const RENO_TRIO  = 4.2;   // three
const RENO_BASE  = 14;    // per-zone cost at the widest team gap
...
defineTypes(Station, { id:"string", progress:"number", broken:"boolean" });
...
class OfficeRoom extends Room {
  ...
  resetObjectives() {
    ...
    for (const st of STATIONS) {
      const s2 = new Station(); s2.id = st.id; s2.progress = 0; s2.broken = false;
      this.state.stations.set(st.id, s2);
    }
    this.state.players.forEach(p => { p.cooldown = 0; p.atPad = ""; p.padKind = ""; });
  }
  ...
  objectivesTick(dt) {
    ...
    st.faults = 0;
    st.stations.forEach(s2 => { if (s2.broken) st.faults++; });
    ...
    for (const r of RENOS) {
      ...
      st.players.forEach(p => {
        ...
        if (near(p, r)) { crew++; p.atPad = r.id; p.padKind = "reno"; }
      });
    ...
    for (const stn of STATIONS) {
      ...
      // a full crew works at full speed; alone on a 2-person job is 3x slower
      const rate = crew.length >= stn.need ? 1 : (crew.length / stn.need) * (2 / 3);
      s2.progress += dt * rate;
      for (const p of crew) { p.atPad = stn.id; p.padKind = "station"; }

      if (s2.progress >= stn.brk - 1e-6) {
        s2.broken = true;
        s2.progress = 0;
        for (const p of crew) { p.cooldown = EMP_CD; p.atPad = ""; p.padKind = ""; }
        console.log(`${stn.n} broken`);
      }
    }

    // IT repairs by standing on a broken station (instant for now;
    // the timed repair + Schelle emails arrive in a later milestone)
    for (const stn of STATIONS) {
      const s2 = st.stations.get(stn.id);
      ...
      st.players.forEach(p => {
        if (p.team !== "it" || p.spectator || !p.charId) return;
        if (near(p, stn)) fixer = p;
      });
      if (fixer) {
        s2.broken = false; s2.progress = 0;
        console.log(`${fixer.name} fixed ${stn.n}`);
      }
    }
  }
