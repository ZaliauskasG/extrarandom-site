...
defineTypes(Player, {
  ...
  color: "string",
  name: "string",
  isIt: "boolean",      // wears the ring
  tagCooldown: "number", // brief immunity so you can't be tagged straight back
});
...
const TAG_RADIUS = 30;      // dots are 28px across, so this is "touching"
const TAG_IMMUNITY = 1.5;   // seconds before the new "it" can tag back
...
class DotsRoom extends Room {
  ...
  onJoin(client) {
    ...
    // first person in the room is "it"; nobody to chase yet, but the
    // ring is there waiting for the second player to arrive
    if (![...this.state.players.values()].some(p => p.isIt)) {
      player.isIt = true;
      player.tagCooldown = TAG_IMMUNITY;
    }
  ...
  onLeave(client) {
    ...
    // don't let "it" vanish with the room -- pass it on
    if (wasIt) {
      const next = this.state.players.values().next().value;
      if (next) { next.isIt = true; next.tagCooldown = TAG_IMMUNITY; }
    }
  ...
  // Tag is a referee decision, so it happens here on the server -- a
  // client never gets to declare "I tagged you".
  checkTags(dt) {
    let it = null;
    this.state.players.forEach((p) => {
      if (p.tagCooldown > 0) p.tagCooldown = Math.max(0, p.tagCooldown - dt);
      if (p.isIt) it = p;
    });
    if (!it || it.tagCooldown > 0) return;

    this.state.players.forEach((target) => {
      if (target === it || target.isIt) return;
      const dist = Math.hypot(target.x - it.x, target.y - it.y);
      if (dist <= TAG_RADIUS) {
        it.isIt = false;
        target.isIt = true;
        target.tagCooldown = TAG_IMMUNITY; // no instant tag-backs
        console.log(`${it.name} tagged ${target.name}`);
        it = target; // whoever we just tagged is now "it" for this pass
      }
    });
  }
}
