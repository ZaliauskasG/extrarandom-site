...
(function (root, factory) {
  ...
  function makeActor(role, x) {
    return {
      ...
      jumpY: 0, jumpVel: 0,
      in: { l: false, r: false, a: false, aEdge: false, jEdge: false }
    };
  }
  ...
  function tryShake(W, a) {
    if (a.role !== "gin" || a.stun > 0 || a.shaking > 0 || a.jumpY > 0) return;
  ...
  function tryJump(a) {
    if (a.role !== "gin" || a.stun > 0 || a.jumpY > 0 || a.jumpVel !== 0) return;
    a.jumpVel = C.JUMP_V0;
  }
  ...
  function botThink(W, role, dt) {
    ...
    b.in.l = b.in.r = false; b.in.aEdge = false; b.in.jEdge = false;
    if (b.stun > 0) return;
    ...
    let jump = false;
    if (b.role === "gin" && b.jumpY <= 0 && b.jumpVel === 0) {
      // jump if a can is closing in and will reach him soon -- not the
      // instant one appears, so it reads as a reaction rather than a cheat
      const incoming = W.shots.find(s => {
        const approaching = (s.x < b.x && s.vx > 0) || (s.x > b.x && s.vx < 0);
        return approaching && Math.abs(s.x - b.x) < 130 && Math.abs(s.x - b.x) > 20;
      });
      if (incoming && W.R() < 0.85) jump = true;
    }

    if (want < 0) b.in.l = true;
    if (want > 0) b.in.r = true;
    if (act) b.in.aEdge = true;
    if (jump) b.in.jEdge = true;
  }
  ...
  function step(W, dt) {
    ...
    [W.gin, W.emily].forEach(p => {
      if (p.stun > 0) { p.moving = false; p.in.aEdge = false; p.in.jEdge = false; return; }
      ...
      if (p.in.jEdge) { tryJump(p); p.in.jEdge = false; }
    });
    ...
    [W.gin, W.emily].forEach(p => {
      ...
      if (p.jumpY > 0 || p.jumpVel !== 0) {
        p.jumpY += p.jumpVel * dt / 1000;
        p.jumpVel -= C.JUMP_G * dt / 1000;
        if (p.jumpY <= 0) { p.jumpY = 0; p.jumpVel = 0; }
      }
      if (p.shaking > 0) {
        p.shaking -= dt;
        if (p.shaking <= 0) {
    ...
    for (let i = W.shots.length - 1; i >= 0; i--) {
      ...
      const ginTop = C.GROUND - W.gin.jumpY - C.CH_H;
      const ginBot = C.GROUND - W.gin.jumpY;
      if (Math.abs(s.x - W.gin.x) < 24 && s.y > ginTop && s.y < ginBot) {
  ...
  function encode(W) {
    const a = p => [Math.round(p.x), p.facing, p.moving ? 1 : 0, Math.round(p.stun),
                    ...
                    Math.round(p.anim), Math.round(p.jumpY)];
    return {
      g: a(W.gin),
      e: a(W.emily),
